...
struct TestConfiguration {
    ...
    struct Timeouts {
        ...
        static let elementDisappear: TimeInterval = 15
        static let navigation: TimeInterval = 10
        static let contentLoad: TimeInterval = 15
        static let animation: TimeInterval = 5
        static let rotation: TimeInterval = 5

        // CI-specific longer timeouts
        static let ciElementAppear: TimeInterval = 60
        static let ciContentLoad: TimeInterval = 30
        static let ciNavigation: TimeInterval = 20

        // Helper to get appropriate timeout based on environment
        static func elementTimeout() -> TimeInterval {
            return ProcessInfo.processInfo.environment["CI"] != nil ? ciElementAppear : elementAppear
        }

        static func contentTimeout() -> TimeInterval {
            return ProcessInfo.processInfo.environment["CI"] != nil ? ciContentLoad : contentLoad
        }

        static func navigationTimeout() -> TimeInterval {
            return ProcessInfo.processInfo.environment["CI"] != nil ? ciNavigation : navigation
        }
    }
    ...
    struct Validation {
        ...
        static let scrollPerformanceMax: TimeInterval = 5.0

        // CI-specific more lenient validation
        static let ciScrollPerformanceMax: TimeInterval = 10.0

        // Helper to get appropriate performance threshold
        static func scrollPerformanceThreshold() -> TimeInterval {
            return ProcessInfo.processInfo.environment["CI"] != nil ? ciScrollPerformanceMax : scrollPerformanceMax
        }
    }
...
extension TestConfiguration {
    ...
    static func isSmallScreen() -> Bool {
        // Use UIDevice for device detection instead of XCUIScreen
