...
class MoreTabUITests: ThriveUITestBase {
    ...
    func testExpectedMoreOptions() {
        ...
        for option in expectedOptions {
            ...
            if cellWithOption.exists {
                ...
                // Verify subtitle exists for this option
                let cell = cellWithOption.firstAncestor(withType: .cell)
                let staticTexts = cell.staticTexts
                XCTAssertGreaterThan(staticTexts.count, 1, "Cell should have title and subtitle")
            }
        }
    ...
    func testSocialActionSheet() {
        ...
        if socialCell.exists {
            let cell = socialCell.firstAncestor(withType: .cell)
            cell.tap()
    ...
    func testSettingsNavigation() {
        ...
        if settingsCell.exists {
            let cell = settingsCell.firstAncestor(withType: .cell)
            cell.tap()
    ...
    func testCellInteraction() {
        ...
        if cells.count > 0 {
            ...
            if imNewCell.exists {
                let cell = imNewCell.firstAncestor(withType: .cell)
                cell.tap()
